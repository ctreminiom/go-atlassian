...
func TestWorkflowService_Gets(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			endpoint:           "/rest/api/3/workflow/search?expand=transitions%2Ctransitions.rules%2Cdefault&isActive=bool&maxResults=50&orderBy=name&queryString=name&startAt=0&workflowName=workflow+name+1&workflowName=workflow+name+2",
			context:            context.Background(),
